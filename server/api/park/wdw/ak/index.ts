...
async function load(event: H3Event) {
    const data: any = await generateWDW(event, WaltDisneyWorldParkID.Animal_Kingdom);
    ...
    const scheduleDat: any = await generateWDWSchedule(event, WaltDisneyWorldParkID.Animal_Kingdom);
